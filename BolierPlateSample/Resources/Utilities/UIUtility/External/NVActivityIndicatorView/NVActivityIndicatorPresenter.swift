...
public class NVActivityIndicatorPresenter {
    ...
    private func show(with activityData: ActivityData)
    {
        ...
        activityContainer.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.1)
        ...
        // Customised By Vasant Hugar
        let svWidth = activityContainer.frame.size.width / 3
        let svHeight = actualSize.height + 30 + 30
        let subActivityContainer: UIView = UIView(frame: CGRect(x: 0, y: 0, width: svWidth, height:svHeight))
        subActivityContainer.center = activityContainer.center
        subActivityContainer.backgroundColor =  UIColor(white: 1, alpha: 0) //Helper.UIColorFromRGB(rgbValue: 0x23c590)
        subActivityContainer.layer.cornerRadius = 5
        subActivityContainer.layer.masksToBounds = true
        activityContainer.addSubview(subActivityContainer)
